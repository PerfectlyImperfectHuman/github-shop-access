    ...
    <div className="space-y-5 animate-fade-in">
      ...
      {lowStock.length > 0 && (
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          className="bg-warning/10 rounded-xl p-4 border border-warning/20"
        >
          ...
          <div className="flex flex-wrap gap-2">
            {lowStock.map((p) => (
              <button
                key={p.id}
                onClick={() => startEdit(p)}
                className="text-xs bg-warning/20 text-warning px-2.5 py-1 rounded-lg font-medium hover:bg-warning/30 transition"
              >
            ...
            <div className="mt-3 px-3 py-2 bg-primary/10 rounded-lg">
              <p className="text-xs text-primary">
                ...
                <span className="font-semibold">
                  {formatCurrency(form.price - form.costPrice)}
                </span>{" "}
                per {form.unit}
                <span className="ml-2 text-muted-foreground">
                  (
                  {(
                    ((form.price - form.costPrice) / form.costPrice) *
                    100
                  ).toFixed(1)}
                  %)
                </span>
          ...
          <div className="px-6 py-14 text-center">
            ...
            {products.length === 0 && (
              <div className="mt-4 flex gap-2 justify-center">
                ...
                <button
                  onClick={() => setShowScanner(true)}
                  className="px-4 py-2 bg-secondary text-secondary-foreground rounded-lg text-sm font-medium hover:opacity-90 transition flex items-center gap-1.5"
                >
            ...
            {filtered.map((p) => {
              ...
              const profitMargin =
                p.price > 0 && p.costPrice > 0
                  ? (((p.price - p.costPrice) / p.costPrice) * 100).toFixed(0)
                  : null;
              return (
                <div
                  key={p.id}
                  className={`flex items-center gap-3 px-4 lg:px-5 py-3.5 hover:bg-muted/20 transition-colors ${!p.isActive ? "opacity-50" : ""}`}
                >
                  <div
                    className={`w-10 h-10 rounded-lg flex items-center justify-center text-sm font-bold shrink-0 ${isLowStock ? "bg-warning/10 text-warning" : "bg-primary/10 text-primary"}`}
                  >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {isLowStock && (
                        <span className="text-[10px] bg-warning/10 text-warning px-1.5 py-0.5 rounded flex items-center gap-0.5 shrink-0">
