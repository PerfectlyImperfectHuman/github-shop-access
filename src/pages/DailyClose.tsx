...
import { db, addExpense, deleteExpense } from "@/lib/db";
...
export default function DailyClose() {
  ...
  const shopPhone = settingsRow?.phone || "";
  const receiptFooter = settingsRow?.receiptFooter || "Thank you!";

  // ── Reactive daily data — auto-refreshes whenever db changes ───────────────
  const dailyData = useLiveQuery(async () => {
    ...
    const [allTxns, allExpenses, allCustomers] = await Promise.all([
      db.transactions.toArray(),
      db.expenses.toArray(),
      db.customers.toArray(),
    ]);

    const dayTxns = allTxns.filter((txn) => {
    ...
    const enriched: EnrichedTxn[] = dayTxns.map((txn) => ({
      ...txn,
      customerName: txn.customerId ? custMap.get(txn.customerId) : undefined,
    }));

    const summary = {
      date,
      cashSales: dayTxns
        .filter((tt) => tt.type === "sale")
        .reduce((s, tt) => s + tt.amount, 0),
      creditGiven: dayTxns
        .filter((tt) => tt.type === "credit")
        .reduce((s, tt) => s + tt.amount, 0),
      paymentsReceived: dayTxns
        .filter((tt) => tt.type === "payment")
        .reduce((s, tt) => s + tt.amount, 0),
      purchases: dayTxns
        .filter((tt) => tt.type === "purchase")
        .reduce((s, tt) => s + tt.amount, 0),
      supplierPayments: dayTxns
        .filter((tt) => tt.type === "supplier_payment")
        .reduce((s, tt) => s + tt.amount, 0),
      expenses: dayExps.reduce((s, e) => s + e.amount, 0),
      salesCount: dayTxns.filter((tt) => tt.type === "sale").length,
      creditCount: dayTxns.filter((tt) => tt.type === "credit").length,
      paymentCount: dayTxns.filter((tt) => tt.type === "payment").length,
      purchaseCount: dayTxns.filter((tt) => tt.type === "purchase").length,
      supplierPaymentCount: dayTxns.filter(
        (tt) => tt.type === "supplier_payment",
      ).length,
      expenseCount: dayExps.length,
    };

    return { summary, transactions: enriched, expenses: dayExps };
  }, [date]);

  const loading = dailyData === undefined || settingsRow === undefined;
  ...
  const closingCash =
    opening + cashSales + payments - expensesTotal - supplierPayments;
  ...
  const printDate = new Date(date).toLocaleDateString("en-PK", {
    day: "2-digit",
    month: "short",
    year: "numeric",
  });
  const printTime = new Date().toLocaleTimeString("en-PK", {
    hour: "2-digit",
    minute: "2-digit",
  });
  ...
  function sendWhatsApp() {
    const lines = [
      ...
      ...(supplierPayments > 0
        ? [`🚚 Supplier Paid: Rs. ${supplierPayments.toLocaleString()}`]
        : []),
      ``,
      ...
      <div className="receipt-print-area hidden print:block">
        ...
        >
          <div style={{ textAlign: "center", marginBottom: "8px" }}>
            ...
            <div
              style={{ fontSize: "12px", fontWeight: "bold", marginTop: "6px" }}
            >
              {t("daily_close_report")}
            </div>
          ...
          {[
            ["Opening Cash", `Rs. ${opening.toLocaleString()}`],
            [
              `Cash Sales (${summary?.salesCount || 0})`,
              `+ Rs. ${cashSales.toLocaleString()}`,
            ],
            [
              `Payments (${summary?.paymentCount || 0})`,
              `+ Rs. ${payments.toLocaleString()}`,
            ],
            [
            ...
            ...(supplierPayments > 0
              ? [
                  [
                    "Supplier Paid",
                    `− Rs. ${supplierPayments.toLocaleString()}`,
                  ],
                ]
              : []),
          ].map(([k, v]) => (
          ...
          >
            <span>Udhar Given ({summary?.creditCount || 0})</span>
            <span>Rs. {credit.toLocaleString()}</span>
          </div>
          {expenses.length > 0 && (
            <>
              <div style={{ borderTop: "1px dashed #000", margin: "6px 0" }} />
      ...
      <div className="space-y-4 animate-fade-in print:hidden max-w-lg mx-auto">
        ...
        <div className="flex items-center gap-2 bg-card rounded-xl border border-border px-3 py-2.5 shadow-sm">
          ...
          <div className="flex-1 text-center min-w-0">
            ...
            {isToday && (
              <p className="text-[11px] text-primary font-medium leading-none mt-0.5">
                {t("today")}
              </p>
            )}
          </div>
          ...
          <>
              ...
              {[
                ...
                >
                  <div className="flex items-start justify-between mb-2">
                    <p className="text-xs text-muted-foreground font-medium leading-tight pr-1">
                      {c.label}
                    </p>
                  ...
                  <p className={`text-xl font-display font-bold ${c.color}`}>
            ...
            >
              <div>
                ...
                <p className="text-xl font-display font-bold text-destructive mt-0.5">
            ...
            <div className="bg-card rounded-xl border border-border shadow-sm overflow-hidden">
              ...
              {showExpenseForm && (
                <form
                  ...
                  className="p-4 border-b border-border bg-muted/20 space-y-2"
                >
                  ...
                  <input
                    ...
                    placeholder={t("expense_note")}
                    className="w-full px-3 py-2 rounded-lg border border-input bg-background text-sm focus:outline-none focus:ring-2 focus:ring-ring"
                  />
                  <div className="flex gap-2">
                    <button
                      type="submit"
                      className="flex-1 py-2 bg-primary text-primary-foreground rounded-lg text-xs font-medium hover:opacity-90 transition"
                    >
                      {t("save")}
                    </button>
                    <button
                      ...
                      className="px-3 py-2 bg-secondary text-secondary-foreground rounded-lg text-xs font-medium"
                    >
                      {t("cancel")}
                    </button>
                  </div>
                ...
                <div className="divide-y divide-border">
                  {expenses.map((e) => (
                    ...
                    >
                      ...
                      <p className="text-sm font-mono font-semibold text-warning shrink-0">
                    ...
                    >
                      ...
                      <p
                        className={`text-sm font-mono font-semibold shrink-0 ${
                          tt.type === "sale"
                            ? "text-primary"
                            : tt.type === "credit" || tt.type === "purchase"
                              ? "text-destructive"
                              : "text-success"
                        }`}
                      >
                        {tt.type === "payment" || tt.type === "supplier_payment"
                          ? "−"
                          : "+"}
                        {formatCurrency(tt.amount)}
                      </p>
                    </div>
                  ))}
                </div>
