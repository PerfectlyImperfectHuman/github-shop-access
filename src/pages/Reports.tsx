      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        {totalCredit > 0 && (
          <div className="bg-card rounded-xl border border-border p-5 shadow-sm">
            ...
            <ResponsiveContainer width="100%" height={220}>
              <PieChart>
                <Pie data={pieData} cx="50%" cy="50%" innerRadius={55} outerRadius={85} dataKey="value" label={({ name, percent }: Record<string, unknown>) => `${name ?? ''} ${(((percent as number) ?? 0) * 100).toFixed(0)}%`}>
